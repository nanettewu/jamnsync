...
from audio_processing.aws import list_files, upload_file, download_file, upload_take
...
def get_update_delete_project():
    ...
    if not project_id or (project_id and not project_id.isdigit()):
        return abort(HTTPStatus.BAD_REQUEST)
...
def get_update_delete_track():
    ...
    if request.method == 'PUT':
        new_track_name = request.form.get('track_name')
...
def create_take():
    ...
    group_name = track.project.group.group_name
    project_name = track.project.project_name
    track_name = track.track_name
    
    s3_info = upload_take(filename, formatted_time, group_name, project_name, track_name)
    if not s3_info:
        os.remove(filepath) # don't save file
        return f"no take URL found", HTTPStatus.BAD_REQUEST
    
    # add new take to database
    new_take_number = len(track.takes) + 1
    new_take = Take(
        track_id=track_id,
    ...
    try:
        ...
        db.session.rollback()
        return abort(HTTPStatus.BAD_REQUEST)
    return jsonify({'message': f"created take #{new_take_number} for track id {track_id}", 'take': f"{new_take_number}", 's3_info': f"{s3_info}"}), HTTPStatus.CREATED
